...
| update_model_config    | 切换模型         | required string model_name：模型名称<br/>required string base_url：基础 URL<br/>required string api_token：API 令牌                                                                                                                                    | string message：操作结果消息                                                                                                   |                                                                                                                                |
...
curl -X POST "http://127.0.0.1:7788/outline_generation" \
     -H "accept: application/json" \
     -H "Content-Type: application/json" \
     -d '{"requirement": "需求规格说明内容", "rag_content": ""}'
```
