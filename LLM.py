...
class LLM:
    def __init__(self, model_name="qwen-ds", api_token="token-abc123", base_url="http://192.168.2.13:9997/v1"):
        print("初始化LLM")
    ...
    def update_prompt(self, prompt_name, new_prompt, new_dir=None):
        if new_dir:
    ...
    def trucate_history(self, messages, max_length):
        ...
        try:
            ...
            async for chunk in response:
                ...
                if content:
                    ...
                    yield f'{json.dumps({"query": prompt, "answer": full_llm_answer})}\n\n'.encode("utf-8")
    ...
    def update_model_config(self, model_name=None, base_url=None, api_token=None):
        if model_name:
            self.model_name = model_name
        if base_url:
            self.base_url = base_url
